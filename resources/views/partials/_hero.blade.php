
<section
class="relative h-72 bg-white flex flex-col justify-center align-center text-center space-y-4 mb-4"
>
<div
    class="absolute top-0 left-0 w-full h-full opacity-30 bg-no-repeat bg-center"
    style="background-image: url('images/background-image.png')"
></div>

<div class="z-10">
    <h1 class="text-6xl font-bold uppercase color:#696cff">
        &#60&#47&#62<span class="text-black">Jobs</span>
    </h1>
    <p class="text-2xl text-black-200 font-bold my-4">
        Find or post jobs & projects
    </p>
    {{-- <div>
        <a
            href="register.html"
            class="inline-block border-2 border-white text-white py-2 px-4 rounded-xl uppercase mt-2 hover:text-black hover:border-black"
            >Sign Up to List a Gig</a
        >
    </div> --}}
</div>
</section>